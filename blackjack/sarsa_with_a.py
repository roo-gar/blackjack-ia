...
Q_STORAGE_FILE = "q_table_sarsa_with_a.json"
...
class QMatrix:
    ...
    def __init__(self, entries=None):
        ...
        if entries is not None:
            ...
            self.entries = []
            for p in range(4, 22):
                self.entries.append(QMatrixEntry(State(p, 'A'), HIT, 0))
                self.entries.append(QMatrixEntry(State(p, 'A'), STAND, 0))
                self.entries.append(QMatrixEntry(State(p, 'A'), DOUBLE, 0))
                for q in range(2, 11):
            ...
            for p in range(2, 22):
                self.entries.append(QMatrixEntry(State('A' + str(p), 'A'), HIT, 0))
                self.entries.append(QMatrixEntry(State('A' + str(p), 'A'), STAND, 0))
                self.entries.append(QMatrixEntry(State('A' + str(p), 'A'), DOUBLE, 0))
                for q in range(2, 11):
                    self.entries.append(QMatrixEntry(State('A' + str(p), q), HIT, 0))
                    self.entries.append(QMatrixEntry(State('A' + str(p), q), STAND, 0))
                    self.entries.append(QMatrixEntry(State('A' + str(p), q), DOUBLE, 0))
...
def get_state(player_hand, dealer_hand):
    ...
    player_hand_contains_a = True if len([card for card in player_hand if card[1] == 'a']) else False
    if player_hand_contains_a:
        player_hand_value = 'A' + str(player_hand_value)

    dealer_hand_value = 'A' if dealer_hand[1] == 'a' else get_card_value(dealer_hand[1:len(dealer_hand)])
...
def update_matrix(new_state, new_action):
    ...
    if previous_hand_result == NO_RESULT_YET:
        entry_to_update.reward = (1 - alpha) * entry_to_update.reward + alpha * math.floor(
            get_reward(previous_hand_result) + GAMMA * QM.get_entry(new_state, new_action).reward - entry_to_update.reward)
    else:
...
def get_action(player_hand, dealer_hand):
    ...
    update_matrix(current_state, current_action)
...
def on_game_start(handsPlayed, funds):
    ...
    if handsPlayed % 100 == 0:
        with open("logs/funds-sarsa-with-a.txt", "a") as file: 
            file.write("{}\n".format(funds))
